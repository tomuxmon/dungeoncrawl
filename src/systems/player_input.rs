...
pub fn player_input(
    ...
    if let Some(key) = *key {
        let delta = match key {
            ...
            VirtualKeyCode::G => pick_item(ecs, commands),
        ...
        // NOTE: player should skip his turn if he presses something else.
...
fn pick_item(ecs: &mut SubWorld, commands: &mut CommandBuffer) -> Point {
    let (player, player_pos) = <(Entity, &Point)>::query()
        .filter(component::<Player>())
        .iter(ecs)
        .find_map(|(pe, pp)| Some((*pe, *pp)))
        .unwrap();
    ...
    items
        ...
        .for_each(|(ie, _, _)| {
            commands.remove_component::<Point>(*ie);
            commands.add_component(*ie, Carried(player));
            if let Ok(e) = ecs.entry_ref(*ie) {
